...
| P0-6 | 恢复生产配置安全默认值 | 默认配置不再内置 API Key 或生产密码，必填凭据从环境变量注入 |
...
| P2-8 | Profile 分层配置 | 新增 dev/prod/postgres profiles，PostgreSQL profile 使用 Flyway + `ddl-auto: validate` |
...
- **状态**: 🟩 已完成
- **已完成**:
  - [x] 默认配置不内置真实 API Key、生产密码或弱密码策略
  - [x] `application-dev.yml` / `application-prod.yml` / `application-postgres.yml` 均通过环境变量接收 DeepSeek 和账号配置
  - [x] `DOCUMIND_ADMIN_PASSWORD` 仍是启动必填项，缺失时应用失败启动
  - [x] 本地和 CI 测试使用测试 profile 或测试属性，不复用生产凭据
- **影响文件**: `application.yml`, `application-dev.yml`, `application-prod.yml`, `application-postgres.yml`, `SECURITY.md`, `DEPLOYMENT.md`

### P0-7 企业级持久化与可迁移存储方案

- **状态**: 🟨 进行中
- **已完成**:
  - [x] 增加 PostgreSQL JDBC 和 Flyway 依赖
  - [x] 新增 `application-postgres.yml`，使用 Flyway migration 初始化关系型 schema，并让 Hibernate 以 `ddl-auto: validate` 校验
  - [x] 新增 `docker-compose.postgres.yml` 供本机验证
  - [x] 新增 `PostgresIntegrationIT`，覆盖账号、文档元数据、知识缺口、审计和知识库权限路径
  - [x] CI 增加 PostgreSQL service container 集成测试
- **仍未完成**:
  - [ ] H2 历史数据到 PostgreSQL 的自动迁移工具
  - [ ] 本地文档目录到对象存储的抽象和迁移
  - [ ] `.documind-vectors.json` 到外置向量库或托管向量服务的迁移
  - [ ] 多实例分布式锁和索引任务协调
- **影响文件**: `pom.xml`, `application-postgres.yml`, `db/migration/V1__init_schema.sql`, `docker-compose.postgres.yml`, `PostgresIntegrationIT`, `.github/workflows/ci.yml`, `DEPLOYMENT.md`
...
- **状态**: 🟩 已完成
- **已完成**:
  - [x] `application-dev.yml`：本地 H2 内存库，适合开发快速启动
  - [x] `application-prod.yml`：H2 文件库单实例部署，WARN 日志级别和连接池调优，默认 `ddl-auto: update`
  - [x] `application-postgres.yml`：PostgreSQL + Flyway，`ddl-auto: validate`
  - [x] 通过 `SPRING_PROFILES_ACTIVE` 或 `--spring.profiles.active` 激活
- **影响文件**: `application-dev.yml`, `application-prod.yml`, `application-postgres.yml`
...
- **已完成**:
  ...
  - [x] CI 增加 OWASP dependency-check 扫描步骤，并让高危漏洞失败构建
  - [x] CI 增加 Trivy Docker 镜像扫描报告
...
| Tika vs POI | 当前 Office 文档解析使用 Apache POI；若后续追求复杂格式抽取质量，可重新评估 Tika 或专用解析服务 |
| 嵌入模型局限 | All-MiniLM-L6-V2 是英文模型，中文语义检索质量受限 |
| 无分布式锁 | 当前索引刷新仍适合单实例，多实例部署需要分布式锁或任务协调 |
| 存储外置化未完成 | PostgreSQL 已覆盖关系型数据；原始文档和向量索引仍在本地文件系统 |
