...
默认管理员用户名为 `admin`，可通过 `DOCUMIND_ADMIN_USERNAME` 修改。密码默认至少 12 个字符，可通过 `DOCUMIND_MIN_PASSWORD_LENGTH` 调整。也可以配置只允许提问的普通账号：
...
服务器部署请参考 [DEPLOYMENT.md](./docs/DEPLOYMENT.md)。
