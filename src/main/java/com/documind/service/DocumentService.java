...
public class DocumentService {
    ...
    public DocumentService() {
        ...
        this.gapRepository = null;
    }

    @Autowired
    public DocumentService(DocumentFileRepository fileRepository, KnowledgeGapRepository gapRepository) {
    ...
    public String storeFile(MultipartFile file, String knowledgeBase) {
        return storeFile(file, knowledgeBase, null, null);
    }

    @Transactional
    public String storeFile(MultipartFile file, String knowledgeBase, String owner, String uploadedBy) {
        ...
        try {
            ...
            if (fileRepository != null) {
                List<DocumentFileEntity> existing = fileRepository.findByKnowledgeBase(kb);
                for (DocumentFileEntity e : existing) {
                    if (fileHash != null && fileHash.equals(e.getFileHash())) {
                        throw new InvalidFileException("该知识库中已存在相同内容的文件: " + e.getFileName());
                    }
                }
            }
            ...
            if (fileRepository != null) {
                DocumentFileEntity entity = fileRepository
                        .findByKnowledgeBaseAndFileName(kb, sanitizedFilename)
                        .orElseGet(() -> new DocumentFileEntity(kb, sanitizedFilename, 0, null, null, null, null, null, STATUS_PENDING, 0, null));
                entity.setSizeBytes(file.getSize());
                entity.setContentType(file.getContentType());
                entity.setOwner(firstNonBlank(normalizedOwner, normalizedUploadedBy));
                entity.setUploadedBy(normalizedUploadedBy);
    ...
    public List<DocumentFileInfo> listDocumentFiles() {
    ...
    public List<DocumentFileInfo> listDocumentFiles(String knowledgeBase) {
        try {
            init();
            String kb = sanitizeKnowledgeBase(knowledgeBase);

            if (fileRepository != null) {
                return fileRepository.findByKnowledgeBase(kb).stream()
                        .map(this::toDto)
                        .map(this::withStaleInfo)
            ...
            return Files.list(directory)
                    ...
                    .map(name -> withStaleInfo(discoveredFileInfo(kb, directory.resolve(name))))
                    .sorted(Comparator.comparing(DocumentFileInfo::getFileName))
                    .collect(Collectors.toList());
        } catch (IOException e) {
    ...
    public List<String> listKnowledgeBases() {
        if (fileRepository != null) {
            ...
            kbs.add(DEFAULT_KNOWLEDGE_BASE);
            fileRepository.findDistinctKnowledgeBase().stream()
                    .filter(kb -> !DEFAULT_KNOWLEDGE_BASE.equals(kb))
                    .sorted()
                    .forEach(kbs::add);
    ...
    public List<KnowledgeBaseStatus> listKnowledgeBaseStatuses() {
        return listKnowledgeBases()
                .stream()
                .map(kb -> {
                    ...
                    return new KnowledgeBaseStatus(kb, files.size(), indexed, pending, indexing, failed, stale, gaps);
                })
                .collect(Collectors.toList());
    }
    ...
    public Path resolveDownloadPath(String filename, String knowledgeBase) {
    ...
    @Transactional
    public boolean deleteFile(String filename, String knowledgeBase) {
        if (filename == null || filename.trim().isEmpty()) {
            throw new InvalidFileException("文件名不能为空");
        }
        ...
        try {
            ...
            if (fileRepository != null) {
                fileRepository.deleteByKnowledgeBaseAndFileName(kb, sanitizedFilename);
            }
    ...
    @Transactional
    public void markIndexing(DocumentFileInfo fileInfo) {
        updateIndexState(fileInfo.getKnowledgeBase(), fileInfo.getFileName(), STATUS_INDEXING, 0, null);
    }

    @Transactional
    public void markIndexed(DocumentFileInfo fileInfo, int chunkCount) {
        updateIndexState(fileInfo.getKnowledgeBase(), fileInfo.getFileName(), STATUS_INDEXED, chunkCount, null);
    }

    @Transactional
    public void markIndexFailed(DocumentFileInfo fileInfo, String error) {
    ...
    @Transactional
    public void recordKnowledgeGap(String knowledgeBase, String question, String sessionId) {
        ...
        try {
            ...
            if (gapRepository != null) {
                KnowledgeGapEntity existing = gapRepository
                        .findByKnowledgeBaseAndQuestionIgnoreCase(kb, normalizedQuestion)
                        .orElse(null);

                if (existing == null) {
                    gapRepository.save(new KnowledgeGapEntity(
                            UUID.randomUUID().toString(), kb, normalizedQuestion,
                            sessionId, now, 1, now));
                } else {
            ...
            logger.error("Failed to record knowledge gap, knowledgeBase={}, questionLength={}",
                    knowledgeBase, question == null ? 0 : question.length(), e);
        }
    }

    public List<KnowledgeGapInfo> listKnowledgeGaps(String knowledgeBase) {
        try {
            String kb = sanitizeKnowledgeBase(knowledgeBase);
            if (gapRepository != null) {
                return gapRepository.findByKnowledgeBaseOrderByLastAskedAtDesc(kb).stream()
                        .map(this::gapToDto)
                        .collect(Collectors.toList());
            }
            return new ArrayList<>();
    ...
    public List<KnowledgeGapInfo> listKnowledgeGaps() {
        return listKnowledgeBases()
                .stream()
    ...
    @Transactional
    public KnowledgeGapInfo resolveKnowledgeGap(String knowledgeBase, String gapId) {
    ...
    public FaqDraftResponse generateFaqDraft(String knowledgeBase) {
        String kb = sanitizeKnowledgeBase(knowledgeBase);
        ...
        markdown.append("# ").append(kb).append(" 知识库 FAQ 草稿\n\n");
        ...
        if (gaps.isEmpty()) {
            ...
            for (int i = 0; i < gaps.size(); i++) {
                ...
                markdown.append("## ").append(i + 1).append(". ").append(gap.getQuestion()).append("\n\n");
                markdown.append("- 出现次数：").append(gap.getOccurrences()).append("\n");
                markdown.append("- 最近提问：").append(gap.getLastAskedAt()).append("\n");
                markdown.append("- 建议负责人：").append(ownerText).append("\n");
    ...
    public List<String> suggestOwners(String knowledgeBase) {
        String kb = sanitizeKnowledgeBase(knowledgeBase);
        ...
        try {
            ...
            if (fileRepository != null) {
                DocumentFileEntity entity = fileRepository
                        .findByKnowledgeBaseAndFileName(kb, sanitizedFilename)
                        .orElseGet(() -> new DocumentFileEntity(kb, sanitizedFilename, 0, null, null, null, null, null, STATUS_PENDING, 0, null));
                entity.setIndexStatus(status);
                entity.setLastIndexedAt(Instant.now().toString());
    ...
    private DocumentFileInfo discoveredFileInfo(String knowledgeBase, Path filePath) {
        try {
            return new DocumentFileInfo(
                    knowledgeBase, filePath.getFileName().toString(),
                    ...
                    null, STATUS_PENDING, 0, null, null);
        } catch (IOException e) {
            return new DocumentFileInfo(
                    knowledgeBase, filePath.getFileName().toString(),
                    0, null, null, null, Instant.now().toString(),
                    null, STATUS_PENDING, 0, null, null);
        }
    }
    ...
    private int countStatus(List<DocumentFileInfo> files, String status) {
        return (int) files.stream().filter(file -> status.equals(file.getIndexStatus())).count();
    }

    private DocumentFileInfo toDto(DocumentFileEntity entity) {
        return new DocumentFileInfo(
                entity.getKnowledgeBase(), entity.getFileName(),
                entity.getSizeBytes(), entity.getContentType(),
                entity.getOwner(), entity.getUploadedBy(),
                entity.getUploadedAt(), entity.getLastIndexedAt(),
                entity.getIndexStatus(), entity.getChunkCount(),
                entity.getError(), entity.getFileHash());
    }

    private KnowledgeGapInfo gapToDto(KnowledgeGapEntity entity) {
        return new KnowledgeGapInfo(
                entity.getId(), entity.getKnowledgeBase(), entity.getQuestion(),
                entity.getSessionId(), entity.getCreatedAt(),
                entity.getOccurrences(), entity.getLastAskedAt());
    }
    ...
    String sanitizeKnowledgeBase(String knowledgeBase) {
